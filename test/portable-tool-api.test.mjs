...
test(acceptance[1], async () => {
  const files = ['src/index.js', 'src/tools/change-tools.js'];
...
test(acceptance[2], async () => {
  ...
  assert.ok(lock.packages?.['node_modules/@deepseek-ai/dsh-tools']);
});
...
test(acceptance[5], async () => {
  const source = await readFile(join(root, 'src/tools/change-tools.js'), 'utf8');
  assert.match(source, /ChangeService|changeService/);
  assert.match(source, /ChangeStore|changeStore/);
  assert.doesNotMatch(source, /authorization\s*=|persistence\s*=|modelBehavior/i);
});
